...
这个仓库是我在 Sui 测试网上实现的 DUCK Protocol，当前包含三个 Move 模块：
...
说明：以上对象 ID 对应我已完成演示流程的上链版本。仓库代码后续有持续迭代，如需与最新代码完全一致，请按第 5 节重新 `publish` 获取新的 `Package ID`。
...
- 本项目当前定位为演示与验证用途的借贷协议。我已经补齐基础清算、风险参数治理、按 epoch 线性计息与链上事件（pledge/borrow/repay/redeem/liquidate/risk updates），但暂未接入预言机与坏账拍卖机制。
- 当前价格假设为固定计价（DUCK 抵押 DUCK），主要用于展示 Move 资产流转、状态约束与清算流程，不等同于生产级金融风控方案。
- `borrow/repay/redeem` 与 `vault` 流程中的 `self_transfer` 是我在当前阶段做的 UX 取舍：资产直接回到交易发起者，便于前端接入；如果后续需要更高可组合性，可以改为返回 `Coin<T>` 由调用方继续编排。
